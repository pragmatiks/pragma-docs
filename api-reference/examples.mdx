...
Upload provider code to build a container image:
...
```json
{
  ...
  "job_name": "buildkit-my-provider-abc123",
  "status": "pending",
...
```bash
curl -H "Authorization: Bearer $TOKEN" \
  "$API/providers/my-provider/builds/buildkit-my-provider-abc123"
```
...
```json
{
  "job_name": "buildkit-my-provider-abc123",
  "status": "success",
  "image": "registry.example.com/my-provider:abc123",
  "error_message": null
}
```
...
```bash
curl -H "Authorization: Bearer $TOKEN" \
  "$API/providers/my-provider/builds/buildkit-my-provider-abc123/logs"
```

### Deploy a Provider

After a successful build, deploy the provider:
